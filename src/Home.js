
function Home() {
  return (
    <div className="container-9 w-container">
      <section
        data-w-id="48c73353-46c9-f2f9-cbca-918a5f64ec08"
        className="hero-heading-right"
      >
        <h3 className="heading-jumbo">
          Hello, I'm Shiva, a{" "}
          <span className="text-span-20"> Software Engineer</span>
        </h3>

        <div className="name-text">
          <span> </span>Also a tech whiz fluent in{" "}
          <span className="text-span-22">React</span> for building awesome user
          interfaces,
          <span className="text-span-22"> Express</span> and{" "}
          <span className="text-span-22">Node.js</span> for powerful backend magic,{" "}
          <span className="text-span-22">JavaScript</span> for crafting interactive
          web experiences, <span className="text-span-22">Python</span> for
          versatile programming adventures and AI-driven solutions,{" "}
          <span className="text-span-22">Java</span> and{" "}
          <span className="text-span-22">Spring Boot</span> for enterprise-level
          wizardry, <span className="text-span-22">.NET Framework</span> for
          scalable wonders,
           <span className="text-span-22"> Angular</span> for slick frontend
          maneuvers, <span className="text-span-22">NoSQL</span> guru with{" "}
          <span className="text-span-22">MongoDB</span>, and diving into the
          exciting realm of <span className="text-span-22">Machine Learning</span>{" "}
          for mind-blowing insights user-centric experiences that drive{" "}
          measurable outcomes.
        </div>

        <div className="text-block-5">
          <h5>
            Currently pursuing Master's in{" "}
            <span className="text-span-21">
              {" "}
              Software Engineering @ San Jose State University{" "}
            </span>{" "}
            and looking for job/internship opportunities.
            <br />
            Previously at <span className="text-span-24">
              {" "}
              ServiceNow{" "}
            </span> and <span className="text-span-24"> NCR Corporation</span>.
          </h5>
        </div>
      </section>
    </div>
  );
}
export default Home;
